...
Let's suppose the following model: $\mathbf{y} = \tilde{\mathbf{X}}\boldsymbol{\beta} +\boldsymbol{\epsilon}$ where $\tilde{\mathbf{X}}$ is the centered matrix, i.e. $\tilde{x}_{ij} = x_{ij} - \bar{x}$. Consequently, $\hat{\beta}_0 = \frac{1}{n}\sum_{i=1}^ny_{i}$. The remaining coefficients get estimated by a ridge regression without intercept, 
...
fit <- glmnet(x_var, y_var, alpha = 0, lambda=c(0.001,1000))
